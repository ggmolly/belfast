...
func TestUpsertBattleSessionCreatesAndUpdates(t *testing.T) {
	...
	if err := UpsertBattleSession(&session); err != nil {
		t.Fatalf("upsert session: %v", err)
	}
	stored, err := GetBattleSession(100)
	if err != nil {
...
func TestDeleteBattleSession(t *testing.T) {
	...
	if err := DeleteBattleSession(200); err != nil {
		t.Fatalf("delete session: %v", err)
	}
	_, err := GetBattleSession(200)
