...
func loadCollectionTemplate(collectionID uint32) (*collectionTemplate, error) {
	entry, err := orm.GetConfigEntry(collectionTemplateCategory, fmt.Sprintf("%d", collectionID))
	if err != nil {
...
func EventCollectionStart(buffer *[]byte, client *connection.Client) (int, int, error) {
	...
	if template.MaxTeam > 0 {
		count, err := orm.GetActiveEventCount(nil, client.Commander.CommanderID)
		if err != nil {
			return 0, 13004, err
		}
	...
	existing, err := orm.GetEventCollection(nil, client.Commander.CommanderID, collectionID)
	if err == nil {
	...
	busy, err := orm.GetBusyEventShipIDs(nil, client.Commander.CommanderID)
	if err != nil {
		return 0, 13004, err
	}
	for _, shipID := range shipIDs {
	...
	if template.Oil > 0 {
		if err := client.Commander.ConsumeResource(2, template.Oil); err != nil {
			return client.SendMessage(13004, &response)
		}
	}
	if existing == nil {
		existing, err = orm.GetOrCreateActiveEvent(nil, client.Commander.CommanderID, collectionID)
		if err != nil {
			return 0, 13004, err
		}
	}
	...
	if err := orm.SaveEventCollection(nil, existing); err != nil {
		return 0, 13004, err
	}
